...
void Histogram::makeHists() {
  ...
  for (short sec = 0; sec < NUM_SECTORS; sec++) {
    ...
    E_vs_theta_e_2nd_reso_events[sec] =
        std::make_shared<TH2D>(Form("E_vs_theta_e_2nd_resonance_region_%d", sec),
                               Form("E_vs_theta_e_2nd_resonance_region_%d", sec), bins, zero, 40.0, bins, zero, p_max);
    ...
    MM2_hist_NPip_events[sec] =
        std::make_shared<TH1D>(Form("MM2_hist_NPip_sec_%d", sec), Form("MM2_hist_NPip_sec_%d", sec), bins, -1.0, 16.0);
...
void Histogram::Fill_x_mu(const std::shared_ptr<Reaction>& _e) {
  ...
  if (_e->onePositive()) {
    ...
    mom_vs_E_x_mu_hist[0]->Fill(_e->P_x_mu(), _e->E_x_mu());
    diff_theta_elec_x_mu_hist[0]->Fill(_e->theta_x_mu() - _e->theta_elec());
    MM2_VS_W_x_mu_hist[0]->Fill(_e->W(), _e->M2_x_mu());
  }
  ...
  if (_e->onePositive_at180_MM0()) {
    ...
    diff_E2_P2_x_mu_hist[3]->Fill(_e->MM2());
    diff_E_P_x_mu_hist[3]->Fill(_e->MM());
    mom_vs_E_x_mu_hist[3]->Fill(_e->P_x_mu(), _e->E_x_mu());
    ...
    MM2_VS_W_x_mu_hist[3]->Fill(_e->W(), _e->M2_x_mu());
    ...
    if (abs(_e->E_x_mu() * _e->E_x_mu()) > 0.1 && abs(_e->P_x_mu() * _e->P_x_mu()) > 0.1 &&
      ...
      diff_E2_P2_x_mu_hist[4]->Fill(_e->MM2());
      diff_E_P_x_mu_hist[4]->Fill(_e->MM());
      mom_vs_E_x_mu_hist[4]->Fill(_e->P_x_mu(), _e->E_x_mu());
      ...
      MM2_VS_W_x_mu_hist[4]->Fill(_e->W(), _e->M2_x_mu());

      if (_e->E_x_mu() > 0.2) {
        E_x_mu_hist[5]->Fill(_e->E_x_mu());
        diff_E2_P2_x_mu_hist[5]->Fill(_e->MM2());
        diff_E_P_x_mu_hist[5]->Fill(_e->MM());
        mom_vs_E_x_mu_hist[5]->Fill(_e->P_x_mu(), _e->E_x_mu());
        ...
        MM2_VS_W_x_mu_hist[5]->Fill(_e->W(), _e->M2_x_mu());
    ...
    if (_e->NPip()) {
      ...
      if (0.8 < _e->MM2_NPip() && _e->MM2_NPip() < 1.0) {
        ...
        if (_e->W() > 1.44 && _e->W() < 1.58) {
          ...
          E_vs_theta_e_3rd_reso_events[all_sectors]->Fill(_e->theta_elec(), _e->E_elec());
